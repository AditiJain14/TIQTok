...
class GaussianMultiheadAttentionWithSource(nn.Module):
    ...
    def __init__(
        ...
        self.k_info_proj = quant_noise(
            nn.Linear(self.kdim, embed_dim, bias=bias), q_noise, qn_block_size
        )
        self.q_info_proj = quant_noise(
            nn.Linear(embed_dim, embed_dim, bias=bias), q_noise, qn_block_size
        )
    ...
    def reset_parameters(self):
        if self.qkv_same_dim:
            ...
            nn.init.xavier_uniform_(self.k_info_proj.weight)
            nn.init.xavier_uniform_(self.q_info_proj.weight)
    ...
    def forward(
        ...
        q_info = q_info.transpose(0, 1)
        k_info = k_info.transpose(0, 1)
        ...
        info_weights = torch.bmm(q_info, k_info.transpose(1, 2))
        info_weights = (
            info_weights.unsqueeze(1)
            .repeat(1, self.num_heads, 1, 1)
            .contiguous()
            .view(bsz * self.num_heads, tgt_len, src_len)
        )
        info_weights = self.apply_sparse_mask(info_weights, tgt_len, src_len, bsz)
        info_weights = torch.sigmoid(info_weights) + 1e-4
        ...
        if train_threshold is not None:
            max_id = (info_weights.cumsum(dim=-1) < train_threshold).sum(
                dim=-1, keepdim=True
            ) + 1
            max_id = torch.cummax(max_id, dim=-2)[0]
            max_id = max_id.max(
                torch.arange(1, tgt_len + 1, device="cuda").unsqueeze(0).unsqueeze(2)
            )
            info_mask = (
                torch.arange(0, src_len, device=max_id.device)
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(bsz * self.num_heads, tgt_len, 1)
            )
            #info_mask gives boolean over which src tokens to mask out
            info_mask = info_mask > max_id
            # attn_weights = attn_weights.masked_fill(info_mask, float("-inf"))

        # test_threshold = 0.4
        if test_threshold is not None:
            max_id = (info_weights.cumsum(dim=-1) < test_threshold).sum(
                dim=-1, keepdim=True
            ) + 1
            max_id = torch.cummax(max_id, dim=-2)[0]
            max_id = max_id.max(
        ...
        if train_threshold is None and test_threshold is None:
            ...
            info_mask = info_mask > src_len

        future_mask = ~info_mask    
        # future_mask = future_mask.sum(dim=-1, keepdim=True)
        import ipdb;ipdb.set_trace()
        # predict incremental step dp
        # dp=[]
        # f_mask = future_mask.sum(dim=-1)
        # for t in range(0,q.shape[1]):
        #     concatenated_input= torch.cat()
        #     dpi=torch.exp(self.vp(torch.tanh(self.wp())))
        # # h = (
        #     q.contiguous()
        ...
        if not (train_threshold is None and test_threshold is None):

            bounded_info = info_weights * ~info_mask
            bounded_info = bounded_info.max(dim=-1)[1]
            p = bounded_info.clamp(1, float(self.max_len))
            ...
            # p = p.clamp(lamda, float(self.max_len))
        ...
        if attn_weights_masked is None:
            attn_weights_masked = attn_weights
        ...
        attn_weights = attn_weights * info_weights

        gaussian_attn_weights = None
        if not (train_threshold is None and test_threshold is None):
            gaussian_attn_weights = torch.mul(attn_weights, alpha)
        
        alfa=0.5
        if gaussian_attn_weights is not None:
            attn_weights = alfa * attn_weights + (1-alfa) * gaussian_attn_weights
        ...
        if step is not None:
            ...
            return (
                attn, 
                attn_weights, 
                info_weights.contiguous().view(bsz, self.num_heads, tgt_len, src_len)[
                :, :1, :, :
                ],
            )
