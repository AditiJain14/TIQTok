...
@register_model("transformer_gma")
class TransformerGMAModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--freeze-pretrained-lm', action='store_true', default=False,
                            help='if True, use path to load pretrained LM Decoder')
        parser.add_argument('--delta', type=float, metavar='D', default=1.0,
                            help='relax offset')
        parser.add_argument(
            "--uni-encoder",
            default=False,
        ...
        if args.add_language_model:
            ...
            if args.share_transformer_lm_decoder_embed:
                ...
                lm_decoder_softmax_weight = nn.Linear(
                    lm_decoder_embed_tokens.weight.shape[1],
                    lm_decoder_embed_tokens.weight.shape[0],
                    bias=False,
                )
                nn.init.normal_(
                    lm_decoder_softmax_weight.weight, mean=0, std=args.decoder_embed_dim ** -0.5
                )

            elif args.share_lm_decoder_softmax_embed:
                lm_decoder_embed_tokens = cls.build_embedding(
                    args, tgt_dict, args.decoder_embed_dim, args.decoder_embed_path
                )
                lm_decoder_softmax_weight = nn.Linear(
                    lm_decoder_embed_tokens.weight.shape[1],
            ...
            elif args.share_transformer_lm_decoder_softmax_embed:
                lm_decoder_embed_tokens = decoder_embed_tokens
                lm_decoder_softmax_weight = nn.Linear(
                    lm_decoder_embed_tokens.weight.shape[1],
                    lm_decoder_embed_tokens.weight.shape[0],
...
class TransformerDecoderGMA(FairseqIncrementalDecoder):
    """
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = TransformerDecoderLayerGMA(args, no_encoder_attn)
    ...
    def forward(
        ...
        x, extra, d = self.extract_features(
            prev_output_tokens,
            encoder_out=encoder_out,
...
@register_model_architecture("transformer_gma", "transformer_tiny_gma")
...
@register_model_architecture("transformer_gma", "transformer_gma")
...
@register_model_architecture("transformer_gma", "transformer_iwslt_de_en_gma")
...
@register_model_architecture("transformer_gma", "transformer_wmt_en_de_gma")
def transformer_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("transformer_gma", "transformer_vaswani_wmt_en_de_big_gma")
...
@register_model_architecture("transformer_gma", "transformer_vaswani_wmt_en_fr_big_gma")
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, "dropout", 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture("transformer_gma", "transformer_wmt_en_de_big_gma")
...
@register_model_architecture("transformer_gma", "transformer_wmt_en_de_big_t2t_gma")
