...
def bulid_weight_matrix_t2t(src_len, tgt_len):
    r = src_len / tgt_len
    ...
    w = w.unsqueeze(0).unsqueeze(1) / (src_len * tgt_len)
    return w


@register_criterion(
    "label_smoothed_cross_entropy_info")
class LabelSmoothedCrossEntropyInfoCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True, num_updates=None):
        ...
        if num_updates is not None:
            train_threshold = 0.2 + 0.8 * math.exp(-num_updates / 5000)
            if num_updates < 1000:
        ...
        net_output, back_net_output, dual_loss, back_data, lm_net_output = model(**sample["net_input"], dual=dual_path, train_threshold=train_threshold)

        info_weights = net_output[1]["extras"]

        info_loss = (
            (info_weights / info_weights.sum(dim=-1, keepdim=True))
            * bulid_weight_matrix_t2t(info_weights.size(-1), info_weights.size(-2))
        ).sum() / info_weights.size(1)
